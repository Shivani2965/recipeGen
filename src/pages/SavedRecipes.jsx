...
import { downloadRecipePdf } from "../services/recipeService.js";
...
const SavedRecipes = () => {
  ...
  const filteredSaved = savedRecipes.filter(
  ...
  const handleDownload = async (recipe) => {
    ...
    try {
      ...
      setNotice({ type: "error", message: "Could not export PDF. Please try again." });
    } finally {
  ...
  const handleRemove = async (recipe) => {
    try {
      ...
      setNotice({ type: "success", message: `Removed "${recipe.title || "recipe"}" from saved.` });
    } catch (err) {
      setNotice({ type: "error", message: "Failed to remove recipe. Please try again." });
    }
  };
  return <div id="saved-recipes-page" className="min-h-screen bg-stone-50 py-8 px-4 sm:px-6 lg:px-8">
    ...
    /* Search within saved */
          ...
          {savedRecipes.length > 0 && <div className="relative w-full sm:w-72">
  ...
  />
    ...
    /* Content */
        ...
        {savedRecipes.length === 0 ? <EmptyState
    type="saved"
    ...
    actionLink="/explore"
  /> : filteredSaved.length === 0 ? <div className="text-center py-12 bg-white rounded-3xl border border-stone-200 p-8">
  ...
  >
                <div>
                  <div className="relative aspect-16/10 bg-stone-100 overflow-hidden">
                    <img
    src={recipe.image || "https://images.unsplash.com/photo-1498837167922-ddd27525d352?w=500"}
    alt={recipe.title}
  ...
  >
                  ...
                  <div className="p-4">
                    ...
                    {recipe.summary && <p className="text-xs text-stone-500 line-clamp-2">
