...
import { getRecentlyViewed, RecentlyViewedItem } from '../services/firestoreService.ts';

const CATEGORIES = [
  { name: 'Quick 30-Min', query: '', tags: 'quick', icon: '⏱️' },
  { name: 'Chicken Dishes', query: 'chicken', tags: '', icon: '🍗' },
  { name: 'Italian Pasta', query: 'pasta', tags: 'italian', icon: '🍝' },
  { name: 'Vegetarian', query: '', tags: 'vegetarian', icon: '🥗' },
  { name: 'Healthy Salads', query: 'salad', tags: 'healthy', icon: '🥑' },
  { name: 'Comfort Soups', query: 'soup', tags: '', icon: '🍲' },
];
...
export const Home: React.FC = () => {
  ...
  // Load featured recipes from Spoonacular
  useEffect(() => {
    let mounted = true;
  ...
  // Load recently viewed recipes if user is logged in
  useEffect(() => {
    if (currentUser) {
      getRecentlyViewed(currentUser.uid)
        .then((items) => setRecentViews(items))
        .catch((e) => console.warn('Could not fetch recent views:', e));
    } else {
      setRecentViews([]);
    }
  }, [currentUser]);
  ...
  return (
    <div id="home-page-container" className="min-h-screen bg-stone-50 text-stone-900">
      {/* Hero Section */}
      ...
      {currentUser && recentViews.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 border-t border-stone-200">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
