...
const History = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([getSearchHistory(currentUser.uid), getRecentlyViewed(currentUser.uid)]).then(([sList, vList]) => {
      setSearches(sList);
      setViews(vList);
      setLoading(false);
    }).catch((e) => {
  ...
  return <div id="history-page" className="min-h-screen bg-stone-50 py-8 px-4 sm:px-6 lg:px-8">
  ...
  >
          ...
          {searches.length === 0 ? <p className="text-sm text-stone-500 italic py-4">No search history recorded yet.</p> : <div className="divide-y divide-stone-100">
              {searches.map((item, idx) => <div
    key={idx}
  ...
  >
    ...
    /* Section 2: Recently Viewed Recipes */
        ...
        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-stone-200/80 shadow-2xs space-y-4">
          ...
          {views.length === 0 ? <p className="text-sm text-stone-500 italic py-4">
              No viewed recipes yet. Click any recipe card to review details here.
            </p> : <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {views.map((item) => <Link
    key={item.recipeId}
  ...
  >
                  <div className="aspect-16/10 bg-stone-200 overflow-hidden">
                    <img
    src={item.image || "https://images.unsplash.com/photo-1498837167922-ddd27525d352?w=400"}
    alt={item.title}
