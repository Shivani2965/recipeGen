...
const IngredientSelector = ({
  ...
  return <div id="ingredient-selector-component" className="w-full">
  ...
  >
    ...
    /* Selected ingredients pills */
        ...
        {ingredients.length > 0 ? <div className="flex flex-wrap items-center gap-2 pt-1">
  ...
  >
            ...
            {onClear && ingredients.length > 1 && <button
    type="button"
    onClick={onClear}
  ...
  >
          ...
          </div> : <p className="text-xs text-stone-500 italic">No ingredients added yet. Add ingredients from your kitchen above.</p>}

        {
    /* Pantry quick suggestions */
  }
        {!compact && <div className="pt-2 border-t border-stone-100">
            <div className="flex items-center gap-1.5 text-xs text-stone-500 mb-2">
              <Sparkles className="w-3.5 h-3.5 text-amber-500" />
              <span>Quick add pantry staples:</span>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {COMMON_PANTRY.filter((item) => !ingredients.includes(item)).slice(0, 8).map((item) => <button
    key={item}
    type="button"
    onClick={() => onAdd(item)}
