...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-stone-200/80 shadow-2xs">
              ...
              {currentUser && (
                <>
                  ...
                  <NavLink to="/favorites" className={navItemClasses}>
                    ...
                    <span>Favorites</span>
                    {favorites.length > 0 && (
                      <span className="ml-0.5 px-1.5 py-0.2 bg-rose-100 text-rose-800 text-[11px] font-bold rounded-full">
                        {favorites.length}
                      </span>
                    )}
                  </NavLink>
                  <NavLink to="/saved" className={navItemClasses}>
                    ...
                    {savedRecipes.length > 0 && (
                      <span className="ml-0.5 px-1.5 py-0.2 bg-amber-100 text-amber-800 text-[11px] font-bold rounded-full">
                        {savedRecipes.length}
                      </span>
                    )}
                  </NavLink>
                ...
                >
                  <div className="w-7 h-7 rounded-full bg-emerald-100 border border-emerald-300 text-emerald-800 flex items-center justify-center text-xs font-bold uppercase overflow-hidden">
                    {userProfile?.photoURL ? (
                      ...
                      userProfile?.name?.charAt(0) || currentUser.displayName?.charAt(0) || <User className="w-3.5 h-3.5" />
                    )}
                  </div>
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div id="mobile-navigation-drawer" className="md:hidden border-t border-stone-200 bg-white px-4 pt-3 pb-6 space-y-3 shadow-lg">
