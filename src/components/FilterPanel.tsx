...
import { SearchFilterOptions } from '../services/recipeService.ts';

interface FilterPanelProps {
  filters: SearchFilterOptions;
  onChange: (key: keyof SearchFilterOptions, value: any) => void;
  onClear: () => void;
  onApply?: () => void;
...
export const FilterPanel: React.FC<FilterPanelProps> = ({
  ...
  const content = (
    <div className="space-y-6 text-stone-800">
          ...
          {TIME_OPTIONS.map((time) => {
            ...
            return (
              <button
                key={time.label}
                type="button"
                onClick={() => onChange('maxReadyTime', isSelected ? undefined : time.value)}
      ...
      {/* Mobile apply button */}
      {onApply && (
        <div className="pt-4 border-t border-stone-200 lg:hidden">
          <button
            type="button"
            onClick={onApply}
